...
print('bob 拥有后1/2的 X 部分')
X_bob_data = FedNdarray(
    partitions={
...
print('bob 拥有完全的 y 部分')
y_bob_data = FedNdarray(
    partitions={
        bob: bob(Divide_y)(y)
    },
    partition_way=PartitionWay.VERTICAL,
...
# 等待IO
wait([p.data for p in X_bob_data.partitions.values()])
wait([p.data for p in y_bob_data.partitions.values()])

print('划分训练集和测试集...')
X2_train, X2_test = train_test_split(X_bob_data, train_size=split_factor, random_state=random_state)
y2_train, y2_test = train_test_split(y_bob_data, train_size=split_factor, random_state=random_state)
...
# 训练模型
print('训练模型...')
model_chooser.train(X2_train, y2_train, params)
...
# 现在结果以密文保存在spu中
spu_yhat = model_chooser.predict(X2_test)
# 转换为明文
yhat = reveal(spu_yhat)

y_ = reveal(y2_test.partitions[bob])
